...
class Database:
    ...
    def delete_queue(self, chat_id):
        with self.connection:
            return self.connection.execute("DELETE FROM `queue` WHERE `chat_id` = ?", (chat_id,))

    def get_chat(self):
        with self.connection:
            chat = self.connection.execute("SELECT * FROM `queue`", ()).fetchmany(1)
            if(bool(len(chat))):
                for row in chat:
                    return row[1]
            else:
